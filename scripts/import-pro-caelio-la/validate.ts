/**
 * Validation for data/pro-caelio-la/.
 *
 *   npx tsx scripts/import-pro-caelio-la/validate.ts
 *
 * Writes data/pro-caelio-la/VALIDATION_REPORT.md, prints a summary, and
 * exits non-zero if any ERROR-level check fails. WARN-level findings
 * (preserved source irregularities) do not fail the run.
 *
 * NOTE: this speech ships Latin-only. Its importer originally also produced
 * an English edition (data/pro-caelio-en/, from English Wikisource rather
 * than Perseus), but independent investigation found that source's
 * translator attribution unreliable - see the commit history and this
 * work's about.json "Known gaps" section - so pro-caelio-en was dropped
 * before shipping and this validator no longer checks it.
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';
import type { Division, GenericWork } from '../../data/pro-caelio-la/types.ts';

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(HERE, '..', '..');
const DIR_LA = join(REPO_ROOT, 'data', 'pro-caelio-la');

const EXPECTED_SECTIONS = 80;
const INCIPIT_LA = 'si quis, iudices, forte nunc adsit';
const EXPLICIT_LA = 'fructus uberes diuturnosque capietis.';
...
function main(): void {
  ...
  const la = loadEdition(DIR_LA, 'pro-caelio-la', 'la', findings);
  ...
  if (!laIncipitOk) err('pro-caelio-la-spot-incipit', `sec-1 does not start with ${JSON.stringify(INCIPIT_LA)} (got: ${JSON.stringify(laFirst.slice(0, 60))})`);
  if (!laExplicitOk) err('pro-caelio-la-spot-explicit', `last section does not end with ${JSON.stringify(EXPLICIT_LA)} (got: ${JSON.stringify(laLast.slice(-60))})`);

  if (!la.anomalies.some((a) => /apparatus|note/i.test(a.note))) err('pro-caelio-la-apparatus-note', 'anomalies.json must document the dropped critical apparatus');
  ...
  writeReport(findings, stats, la.anomalies, {
    incipitOk: laIncipitOk, incipitGot: laFirst.slice(0, 60), explicitOk: laExplicitOk, explicitGot: laLast.slice(-60),
  });
  ...
  process.stdout.write(`  la: ${stats.laSections} sections / ${stats.laChars} chars   (Latin-only edition - no English bundled)\n`);
...
function writeReport(
  findings: Finding[],
  stats: { laSections: number; laChars: number },
  laAnomalies: Anomaly[],
  ...
  const warns = findings.filter((f) => f.level === 'WARN');
  const L: string[] = [];
  L.push('# Pro M. Caelio (Latin only) validation report');
  L.push('');
  L.push(`Generated: ${new Date().toISOString()}`);
  L.push('');
  L.push(`**Result: ${errors.length === 0 ? 'PASS' : 'FAIL'}** - ${errors.length} error(s), ${warns.length} warning(s).`);
  L.push('');
  L.push('**Latin only**: no English translation is bundled for this speech. The only candidate found ' +
    '(English Wikisource, "For Marcus Caelius") had unreliable, partly unattributed provenance on independent ' +
    'investigation (its earliest 2006 revisions read as an original community translation, not a scan ' +
    'transcription of a public-domain published edition) and was dropped before shipping.');
  L.push('');
  L.push('## Counts');
  L.push('');
  ...
  L.push('');
  L.push(`- ${spot.incipitOk ? 'OK' : 'FAIL'} - Latin sec-1 incipit\n  - got: \`${spot.incipitGot}\``);
  L.push(`- ${spot.explicitOk ? 'OK' : 'FAIL'} - Latin final section explicit\n  - got: \`${spot.explicitGot}\``);
  L.push('');
