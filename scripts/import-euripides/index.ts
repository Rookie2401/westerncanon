/**
 * Euripides - all 19 surviving plays, Greek + English. Run-once ingestion.
 *
 *   npm run import:euripides
 *
 * Reads scripts/import-euripides/raw/tlg0006.tlgNNN.perseus-*.xml (already
 * fetched from PerseusDL/canonical-greekLit and committed) and writes, per
 * play per language:
 *   data/euripides-<slug>-grc|en/{work.json,about.json,anomalies.json,types.ts}
 *
...
function main(): void {
  const plays = DRAMA_WORKS.filter((e) => e.playwright === 'euripides');
  process.stdout.write(`Euripides importer - ${plays.length} plays\n\n`);
