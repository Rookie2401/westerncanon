/**
 * Validation for data/politics-en/.
 *
 *   npx tsx scripts/import-politics-en/validate.ts
 *
 * Writes data/politics-en/VALIDATION_REPORT.md, prints a summary, and exits
...
import type { Division, GenericWork } from '../../data/politics-en/types.ts';

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(HERE, '..', '..');
const DIR = join(REPO_ROOT, 'data', 'politics-en');

const WORK_ID = 'politics-en';
const EXPECTED_CHAPTER_COUNTS = [13, 12, 18, 16, 12, 8, 17, 7];
const OPENING_PREFIX = 'As we see that every city is a society, and every society Ed. is established for some good purpose';
const CLOSING_SUFFIX = 'moderation, possibility, and decorum.';

const LEAK_MARKERS = [
  '&amp;', '&lt;', '&gt;', '{{', '}}', '[[', ']]', '<ref', '</ref', '<references',
  'http://', 'https://', "''", '<p>', '</p>', '<div', '[Bekker', '[1252', '[1253',
];
...
function main(): void {
  ...
  if (about['translator'] !== 'William Ellis') err('translator', `about.json translator must be "William Ellis", got ${JSON.stringify(about['translator'])}`);

  const books = work.divisions ?? [];
  if (books.length !== EXPECTED_CHAPTER_COUNTS.length) err('book-count', `expected ${EXPECTED_CHAPTER_COUNTS.length} books, got ${books.length}`);
  ...
  books.forEach((b, i) => {
    ...
    if (b.id !== `book-${i + 1}`) err('book-id', `book[${i}] id ${JSON.stringify(b.id)}, expected "book-${i + 1}"`);
    if (b.number !== String(i + 1)) err('book-number', `${b.id}: number ${JSON.stringify(b.number)}, expected "${i + 1}"`);
    ...
    chapterCounts.push(`Book ${i + 1}: ${chapters.length} chapters`);
    ...
    chapters.forEach((c, j) => {
      ...
      const wantId = `book-${i + 1}-ch-${wantNumber}`;
      ...
      if (typeof c.ref !== 'string' || !/^Bekker \d{3,4}[ab]–\d{3,4}[ab]$/.test(c.ref)) {
        err('chapter-ref', `${c.id}: ref must be a "Bekker <start>–<end>" string, got ${JSON.stringify(c.ref)}`);
      } else {
        markerCount++;
      }
  ...
  // ---- leaked markup / stray Bekker brackets ----
  const leaks: string[] = [];
  ...
  walk(books);
  if (leaks.length) err('no-leaked-markup', `${leaks.length} string(s) contain transport markup or un-stripped Bekker brackets:\n    ${leaks.slice(0, 10).join('\n    ')}`);

  // ---- required anomaly documentation ----
  if (!anomalies.some((a) => /Bekker/i.test(a.note))) err('bekker-note', 'anomalies.json must document the Bekker reference reconstruction');
  if (!anomalies.some((a) => /Ed\./.test(a.note))) err('ed-stray-note', 'anomalies.json must document the preserved stray "Ed." token');
  if (!anomalies.some((a) => /INDEX|Introduction|front.{0,5}back matter/i.test(a.note))) warn('frontmatter-note', 'anomalies.json should document excluded front/back matter');

  // ---- spot checks ----
  const openingText = books[0]?.children[0]?.passages[0]?.text ?? '';
  const okStart = openingText.startsWith(OPENING_PREFIX);
  if (!okStart) err('spot-check-opening', `book-1-ch-1 does not start with ${JSON.stringify(OPENING_PREFIX)} (got: ${JSON.stringify(openingText.slice(0, 140))})`);
...
function writeReport(
  ...
  anomalies: Anomaly[] = [],
  spot?: { openingOk: boolean; openingGot: string; closingOk: boolean; closingGot: string; markerCount: number },
): void {
  const errors = findings.filter((f) => f.level === 'ERROR');
  const warns = findings.filter((f) => f.level === 'WARN');
  const L: string[] = [];
  L.push(`# Politics (English, Ellis) validation report`);
  L.push('');
  ...
  if (spot) {
    ...
    L.push('');
    L.push(`- ${spot.openingOk ? 'OK' : 'FAIL'} - book-1-ch-1 opening\n  - got: \`${spot.openingGot}\``);
    L.push(`- ${spot.closingOk ? 'OK' : 'FAIL'} - final chapter closing\n  - got tail: \`${spot.closingGot}\``);
    L.push(`- chapters with a well-formed Bekker ref: ${spot.markerCount}`);
    L.push('');
  }
