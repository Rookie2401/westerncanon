/**
 * Validation for data/brutus-en/.
 *
 *   npx tsx scripts/import-brutus-en/validate.ts
 */
...
const DIR = join(REPO_ROOT, 'data', 'brutus-en');
...
function main(): void {
  const report = validateGenericWork({
    ...
    dir: DIR,
    expectLang: 'en',
    spotStart: 'When I had left Cilicia, and arrived at Rhodes',
    spotEnd: '"--[Caetera defunt.]',
    ...
    extraChecks: (work, findings, extra) => {
      ...
      // no leftover Gutenberg footnote/underscore markup
      ...
      // chunk sizes shouldn't blow past the target by an unreasonable margin
