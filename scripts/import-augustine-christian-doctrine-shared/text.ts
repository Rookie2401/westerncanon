/**
 * Shared text helpers for the On Christian Doctrine importers (Latin +
 * English). Guiding rule (mirrors scripts/import-isagoge-shared/text.ts and
 * every other importer in this repo): never discard or silently correct
 * source reading text. We only:
...
/**
 * Strip MediaWiki italic/bold markup (runs of two or three apostrophes).
 * This app's Passage.text has no rich-text/formatting model, so the markers
 * are transport scaffolding like `{{templates}}` or `<center>` tags -
 * stripped, while the literal words (and any single apostrophes/quote marks)
 * are kept verbatim. Returns the stripped text and how many marker runs were
 * removed, for a single corpus-level anomaly note.
 */
export function stripQuoteMarkup(input: string): { text: string; count: number } {
  let count = 0;
  const text = input.replace(/'{2,3}/g, () => {
    count += 1;
    return '';
  });
  return { text, count };
