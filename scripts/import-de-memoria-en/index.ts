 ...
 * J. I. Beare's English translation (Oxford, 1908), via English Wikisource.
 * Run-once ingestion pipeline.
 *
 *   npx tsx scripts/import-de-memoria-en/index.ts
 *
 * Reads scripts/import-de-memoria-en/raw/on-memory-and-reminiscence.json
 * (already in the repo — the cached MediaWiki
 * action=query&prop=revisions&rvslots=main&rvprop=content response). Writes
 * data/de-memoria-en/{work,about,anomalies}.json, then run
 * `npx tsx scripts/import-de-memoria-en/validate.ts`.
 *
 * --- What was verified before this importer was written -------------------
 * The page exists and carries real transcribed prose (28 KB); it is not a
 * stub. It is ordinary wikitext, not a page-scan transclusion, so the
 * plain-wikitext technique is used and there are no Bekker markers to import.
 * Both chapters of the standard division are present, complete and
 * untruncated.
 */
  ...
  sections: [
    {
      ...
      paragraphs: [
        "Aristotle's On Memory and Reminiscence in J. I. Beare's English translation, made for the Oxford Works of Aristotle Translated into English and published in 1908 as part of that series' volume III, the Parva Naturalia. Its two chapters distinguish memory — which belongs to the same part of the soul as imagination, and always involves time — from recollection, the deliberate hunt for what one knew, with its account of association by similarity, contrast and contiguity.",
    ...
    },
    {
      ...
      paragraphs: [
        "The machine-readable text is the raw wikitext of the English Wikisource page \"On Memory and Reminiscence\", fetched once and committed under the importer's raw/ directory. This batch's brief expected djvu page-scan transclusions (for which only the rendered HTML carries any text); direct inspection found ordinary wikitext instead, so the simpler wikitext route was used and the difference is disclosed here rather than described as a page-scan provenance it does not have. The text is bundled with the app; nothing is loaded from the network at runtime.",
      ],
    },
    {
      heading: "How it was imported",
      paragraphs: [
        "The page opens with a `{{header}}` template (title, translator, navigation) which is transport furniture and is dropped, and ends with interwiki links and a licence template that are likewise skipped and counted. Chapters are marked by `==Part N==` wiki headings. Every paragraph under a chapter marker, in document order, becomes part of that chapter's single Passage, joined by blank lines.",
    ...
    },
    {
      ...
      paragraphs: [
        "There are no gaps in the text. anomalies.json records the mechanical departures: the furniture lines skipped, the null reference scheme, and the plaintext-rather-than-page-scan provenance.",
...
runWikitextImport({
  ...
  shape: 'flat',
  pages: [
    { rawFile: "on-memory-and-reminiscence.json", pageTitle: "On Memory and Reminiscence", label: "page", expectedChapters: [1, 2] },
  ],
  about,
  ...
  extraAnomalies: [
    ...
    { where: `${WORK_ID} / relation to the Greek sibling`, note: "This English edition was parsed entirely independently of any Greek edition of the same treatise in this library; the two are not forced to agree on chapter boundaries, and no division in de-memoria-en was adjusted to match a Greek text." },
  ],
});
