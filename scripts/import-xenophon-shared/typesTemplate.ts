/**
 * The `types.ts` written into every `data/xenophon-<slug>-{grc,en}/` output
 * directory - byte-identical across all 14 works FOR A GIVEN LANGUAGE (one
 * template for the 14 Greek editions, one for the 14 English editions;
 * mirrors the convention already used across this app, e.g.
 * scripts/import-archimedes-shared/typesTemplate.ts). Kept as one canonical
 * function here rather than hand-duplicated 28 times; scripts/import-xenophon/
 * index.ts writes the result into each work directory on every import run.
 */
export function xenophonTypesFile(lang: 'grc' | 'en'): string {
  const langDesc =
    lang === 'grc'
      ? "'grc' - E. C. Marchant's Greek text (Xenophontis Opera Omnia, Oxford: Clarendon Press, 1900-1921)"
      : "'en' - the Loeb Classical Library English translation (\"Xenophon in Seven Volumes\", Harvard University Press / William Heinemann, 1914-1925)";
  return `/**
 * Type definitions for one bundled Xenophon work corpus in
 * \`data/xenophon-<slug>-${lang}/\`.
 *
 ...
 * across all 14 Xenophon ${lang === 'grc' ? 'Greek' : 'English'} work directories - see
...
export type Lang = ${lang === 'grc' ? "'grc'" : "'en'"};
...
export interface Passage {
  ...
  n: string;
  /** verbatim ${lang === 'grc' ? 'Greek' : 'English'} paragraph text: entities decoded, NFC-normalised, whitespace collapsed */
  text: string;
