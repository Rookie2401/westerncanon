/**
 * Validation for data/pro-marcello-la/.
 *
 *   npx tsx scripts/import-pro-marcello-la/validate.ts
 *
 * Writes data/pro-marcello-la/VALIDATION_REPORT.md, prints a summary, and
 * exits non-zero if any ERROR-level check fails. WARN-level findings
 * (documented source irregularities) do not fail the run.
 *
 * Standalone (mirrors scripts/import-pro-sestio-la/validate.ts).
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';
import type { Division, GenericWork } from '../../data/pro-marcello-la/types.ts';

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(HERE, '..', '..');
const DIR = join(REPO_ROOT, 'data', 'pro-marcello-la');
const WORK_ID = 'pro-marcello-la';
const EXPECTED_SECTIONS = 34;
const EXPECTED_CHAPTERS = 11;
const SPOT_START = 'diuturni silenti, patres conscripti, quo eram his temporibus usus, non';
const SPOT_END = 'quod fieri iam posse non arbitrabar, magnus hoc tuo facto cumulus accesserit.';
...
function findings(): { findings: Finding[]; report: string[] } {
  ...
  if (!anomalies.some((a) => /partial/.test(a.note) && /Wikisource/.test(a.note))) {
    err('anomaly-partial-translation', 'anomalies.json must document the deliberate exclusion of the partial Wikisource English translation');
  }
  ...
  report.push('');
  report.push('## Per-section refs & chars');
  report.push('');
