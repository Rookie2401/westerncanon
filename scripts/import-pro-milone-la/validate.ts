/**
 * Validation for data/pro-milone-la/.
 *
 *   npx tsx scripts/import-pro-milone-la/validate.ts
 *
 * Writes data/pro-milone-la/VALIDATION_REPORT.md, prints a summary, and
...
import type { Division, GenericWork } from '../../data/pro-milone-la/types.ts';

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(HERE, '..', '..');
const DIR = join(REPO_ROOT, 'data', 'pro-milone-la');
const WORK_ID = 'pro-milone-la';
const EXPECTED_SECTIONS = 105;
const EXPECTED_CHAPTERS = 38;
const SPOT_START = 'etsi vereor, iudices, ne turpe sit pro fortissimo viro dicere incipient';
const SPOT_END = 'optimum et sapientissimum et fortissimum quemque delegit.';
...
function findings(): { findings: Finding[]; report: string[] } {
  ...
  if (!anomalies.some((a) => /abbr/.test(a.note) || /choice/.test(a.note))) {
    err('anomaly-abbr', 'anomalies.json must document how <choice>/<abbr>/<expan> was resolved (this witness has 2)');
  }
