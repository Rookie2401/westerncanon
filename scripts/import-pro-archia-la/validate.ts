/**
 * Validation for data/pro-archia-la/ AND data/pro-archia-en/ together (one
 * validator per speech, covering both its Latin and English editions, per
 * this batch's npm-script convention - see scripts/cicero-npm-scripts-group-a.json).
 *
 *   npx tsx scripts/import-pro-archia-la/validate.ts
 *
 * Writes data/pro-archia-la/VALIDATION_REPORT.md, prints a summary, and
 * exits non-zero if any ERROR-level check fails. WARN-level findings
 * (preserved source irregularities) do not fail the run. Modelled on
 * scripts/import-aristotle-posterior-analytics-en/validate.ts.
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';
import type { Division, GenericWork } from '../../data/pro-archia-la/types.ts';

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(HERE, '..', '..');
const DIR_LA = join(REPO_ROOT, 'data', 'pro-archia-la');
const DIR_EN = join(REPO_ROOT, 'data', 'pro-archia-en');

const EXPECTED_SECTIONS = 32;
const INCIPIT_LA = 'si quid est in me ingeni';
const EXPLICIT_LA = 'certo scio.';
const INCIPIT_EN = 'If there is any natural ability in me';
const EXPLICIT_EN = 'I am quite certain.';
...
function main(): void {
  ...
  const la = loadEdition(DIR_LA, 'pro-archia-la', 'la', findings);
  const en = loadEdition(DIR_EN, 'pro-archia-en', 'en', findings);
  ...
  if (!laIncipitOk) err('pro-archia-la-spot-incipit', `sec-1 does not start with ${JSON.stringify(INCIPIT_LA)} (got: ${JSON.stringify(laFirst.slice(0, 60))})`);
  if (!laExplicitOk) err('pro-archia-la-spot-explicit', `last section does not end with ${JSON.stringify(EXPLICIT_LA)} (got: ${JSON.stringify(laLast.slice(-60))})`);
  ...
  if (!enIncipitOk) err('pro-archia-en-spot-incipit', `sec-1 does not start with ${JSON.stringify(INCIPIT_EN)} (got: ${JSON.stringify(enFirst.slice(0, 60))})`);
  if (!enExplicitOk) err('pro-archia-en-spot-explicit', `last section does not end with ${JSON.stringify(EXPLICIT_EN)} (got: ${JSON.stringify(enLast.slice(-60))})`);

  // ---- cross-edition check: Latin and English should have the same section count and numbering ----
  if (la.work && en.work) {
  ...
  if (!la.anomalies.some((a) => /apparatus|note/i.test(a.note))) err('pro-archia-la-apparatus-note', 'anomalies.json must document the dropped critical apparatus');
  if (!en.anomalies.some((a) => /argument/i.test(a.note))) err('pro-archia-en-argument-note', 'anomalies.json must document how "THE ARGUMENT" front matter was handled');
...
function writeReport(
  ...
  const warns = findings.filter((f) => f.level === 'WARN');
  const L: string[] = [];
  L.push('# Pro Archia Poeta (Latin + English) validation report');
  L.push('');
