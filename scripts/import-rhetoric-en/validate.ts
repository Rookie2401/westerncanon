/**
 * Validation for data/rhetoric-en/.
 *
 *   npx tsx scripts/import-rhetoric-en/validate.ts
 *
 * Writes data/rhetoric-en/VALIDATION_REPORT.md, prints a summary, and
...
import type { Division, GenericWork } from '../../data/rhetoric-en/types.ts';

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(HERE, '..', '..');
const DIR = join(REPO_ROOT, 'data', 'rhetoric-en');

const WORK_ID = 'rhetoric-en';
/** book number -> traditionally cited chapter count (I, II, III) */
const EXPECTED_BOOKS = [
  { number: '1', chapters: 15 },
  { number: '2', chapters: 26 },
  { number: '3', chapters: 19 },
];
// Verbatim opening words of book-1 ch-1's single Passage (Freese, 1926).
const INCIPIT = 'Rhetoric is a counterpart of Dialectic; for both have to do with matters that are in a manner within the cognizance of all men';
// Verbatim closing words of book-3 ch-19's single Passage (Freese, 1926) - the
// treatise's well-known final sentence.
const EXPLICIT = 'I have spoken; you have heard; you know the facts; now give your decision.';

const LEAK_MARKERS = [
  '&amp;', '&lt;', '&gt;', '{{', '}}', '[[', ']]', '<ref', '</ref', '<references',
  'http://', 'https://', "''", ':\'\'', '<p>', '</p>', '<div', '===Notes===',
  '<note', '<milestone', '<bibl', '<persName', '<surname', '<term',
];
...
function main(): void {
  ...
  if (about['workId'] !== WORK_ID) err('about-workId', `about.json workId must be "${WORK_ID}", got ${JSON.stringify(about['workId'])}`);
  if (about['translator'] !== 'John Henry Freese') err('translator', `about.json translator must be "John Henry Freese", got ${JSON.stringify(about['translator'])}`);
  if (typeof about['license'] !== 'string' || !/1926/.test(about['license'] as string)) err('license', 'about.json license must cite the 1926 publication year (public-domain basis)');
  ...
  books.forEach((b, i) => {
    ...
    chapterCounts.push(`Book ${spec.number}: ${chapters.length} chapters (traditionally cited ${spec.chapters})`);
    if (chapters.length !== spec.chapters) {
      warn('chapter-count', `${b.id}: expected ${spec.chapters} chapters (traditionally cited), got ${chapters.length}`);
    }
    ...
    chapters.forEach((c, j) => {
      ...
      if (c.ref !== null && !/^[0-9]{4}[a-c](–[0-9]{4}[a-c])?$/.test(c.ref)) {
        err('chapter-ref-format', `${c.id}: ref ${JSON.stringify(c.ref)} does not look like a Bekker page or page range`);
      }
  ...
  if (totalChapters !== 60) err('total-chapter-count', `expected 60 total chapters (15+26+19), got ${totalChapters}`);
  ...
  if (!anomalies.some((a) => /footnote|<note/i.test(a.note))) err('footnote-note', 'anomalies.json must document the dropped Freese footnotes');
  if (!anomalies.some((a) => /Bekker/i.test(a.note))) err('ref-note', 'anomalies.json must document the Bekker reference scheme');
  if (!anomalies.some((a) => /section/i.test(a.note))) err('structure-note', 'anomalies.json must document the extra "section" nesting level');

  // ---- spot checks (verbatim, against the actual parsed text) ----
  const p1 = books[0]?.children[0]?.passages[0]?.text ?? '';
  const okStart = p1.startsWith(INCIPIT);
  if (!okStart) err('spot-check-incipit', `book-1 ch-1 does not start with ${JSON.stringify(INCIPIT)} (got: ${JSON.stringify(p1.slice(0, 140))})`);
  ...
  if (!okEnd) err('spot-check-explicit', `final chapter (${lastCh?.id}) does not end with ${JSON.stringify(EXPLICIT)} (got tail: ${JSON.stringify(lastText.slice(-140))})`);
  ...
  writeReport(findings, stats, chapterCounts, anomalies, { incipitOk: okStart, incipitGot: p1.slice(0, 140), explicitOk: okEnd, explicitGot: lastText.slice(-140) });

  const errors = findings.filter((f) => f.level === 'ERROR');
  const warns = findings.filter((f) => f.level === 'WARN');
  process.stdout.write('\n=== validate:rhetoric-en ===\n');
...
function writeReport(
  ...
  const warns = findings.filter((f) => f.level === 'WARN');
  const L: string[] = [];
  L.push(`# Rhetoric (English, Freese) validation report`);
  L.push('');
  ...
  if (spot) {
    ...
    L.push(`- ${spot.explicitOk ? 'OK' : 'FAIL'} — final chapter (book-3 ch-19) explicit\n  - got: \`${spot.explicitGot}\``);
    L.push('');
  }
