/**
 * Validation for data/athenian-constitution-en/.
 *
 *   npx tsx scripts/import-athenian-constitution-en/validate.ts
 *
 * Writes data/athenian-constitution-en/VALIDATION_REPORT.md, prints a
...
import type { Division, GenericWork } from '../../data/athenian-constitution-en/types.ts';

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(HERE, '..', '..');
const DIR = join(REPO_ROOT, 'data', 'athenian-constitution-en');

const WORK_ID = 'athenian-constitution-en';
const EXPECTED_CHAPTERS = 69;
const OPENING_PREFIX = '...[They were tried] by a court empanelled from among the noble families';
const CLOSING_SUFFIX = 'the jurors receive their pay in the order assigned by the lot.';
...
function main(): void {
  ...
  if (about['translator'] !== 'Sir Frederic G. Kenyon') err('translator', `about.json translator must be "Sir Frederic G. Kenyon", got ${JSON.stringify(about['translator'])}`);
  ...
  if (!anomalies.some((a) => /fragmentary opening|papyrus/i.test(a.note))) err('opening-note', 'anomalies.json must document the papyrus\'s fragmentary opening');
  if (!anomalies.some((a) => /[Tt]ranscriber/i.test(a.note))) err('transcriber-note', 'anomalies.json must document the preserved transcriber\'s note in ch-8');
  ...
  // ---- ch-8 transcriber note preserved verbatim ----
  const ch8 = chapters.find((c) => c.id === 'ch-8');
  const ch8Text = ch8?.passages[0]?.text ?? '';
  if (!ch8Text.includes("[Transcriber's note: of?]")) {
    err('transcriber-note-preserved', `ch-8 must contain the verbatim preserved "[Transcriber's note: of?]" - not found`);
  }
  ...
  process.stdout.write('\n=== validate:athenian-constitution-en ===\n');
...
function writeReport(
  ...
  const warns = findings.filter((f) => f.level === 'WARN');
  const L: string[] = [];
  L.push(`# Athenian Constitution (English, Kenyon) validation report`);
  L.push('');
